...
def mtFeatureExtractionToFile(fileName, midTermSize, midTermStep, shortTermSize, shortTermStep, outPutFile,
    ...
    if not storeStFeatures:
        ...
        if storeToCSV:
            numpy.savetxt(outPutFile+"_lt.csv", stF.T, delimiter=",")    # store st features to CSV file
            print("Short-term CSV file: " + outPutFile + "_lt.csv saved")
...
def mtFeatureExtractionToFileDir(dirName, midTermSize, midTermStep, shortTermSize, shortTermStep, storeStFeatures=False, storeToCSV=False, PLOT=False):
    files = glob.glob(os.getcwd() + os.sep + dirName + '*.wav')
    print(os.getcwd() + os.sep + dirName + '*.wav')
    for f in files:
        outPath = f
